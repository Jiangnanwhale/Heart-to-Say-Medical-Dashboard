...
def show_input_data():
    ...
    with st.form(key='input_form'):
        col1, col2 = st.columns(2)  # Create two columns for better organization
...
def show_upload_dataset():
    with st.sidebar:
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                st.session_state["df"] = df  # Store the dataframe in session state for further use
                st.subheader("Would you like to perform Data Preprocessing before analysis?")
                preprocessing_choice = st.radio("Select an option:", ("Yes, I need", "No, my dataset is ready to be analyzed"))
    ...
    if "uploaded_file" in st.session_state:
        try:
            ...
            st.error('There is no file to analyze. Please upload your dataset or use a default dataset.')
...
def upload_your_model_file(df):
    ...
    if uploaded_file is not None:
        ...
        analysis_option = st.sidebar.selectbox(
            ...
            ("View Predictions", "Model Performance Metrics", "SHAP Analysis", "Upload New Data")
        )
