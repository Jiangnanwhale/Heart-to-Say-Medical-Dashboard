...
def show_dashboard():
        ...
        if uploaded_file is not None:
            ...
            if "preprocessing_needed" not in st.session_state:
                ...
                if preprocessing_choice == "Yes, I need":
                    ...
                    show_preprocessing(pd.read_csv(uploaded_file))  # 传入上传的文件
                else:
                    st.session_state["preprocessing_needed"] = False
                    # 直接进入主选项
                    df = pd.read_csv(uploaded_file)  # 读取上传的文件
                    show_main_options(df, search_query)  # 跳转到主选项
    ...
    if "uploaded_file" in st.session_state:
        ...
        show_main_options(df, search_query)  # 当上传文件后，确保能显示主选项
        ...
        show_main_options(df, search_query)  # 确保使用默认数据时也能显示主选项
    else:
...
def show_main_options(df, search_query):
    ...
    with st.sidebar:
        ...
        option = st.radio("Select an option:", ["Data Overview", "Exploratory Data Analysis", "Data Modeling"])
...
def show_preprocessing(df):
    ...
    st.success("Encode Categorical Variables successfully!")
