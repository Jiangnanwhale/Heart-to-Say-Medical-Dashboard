...
def upload_your_model_file(df):
    ...
    if uploaded_file is not None:
        ...
        if st.sidebar.button("Evaluate Model"):
            ...
            st.dataframe(report_df.style.highlight_max(axis=0), use_container_width=True)  # Highlight max values for better comparison
            # Performance Matrix (Precision, Recall, F1-Score)
            ...
            fig.update_layout(
                ...
                title_font=dict(size=20, color='black'),  # Title font size and color
                xaxis=dict(tickmode='linear'),  # Ensure all ticks are shown
